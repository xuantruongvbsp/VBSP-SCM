...
def tao_file_mau_dgd(ten_pgd: str, dgd_map: dict[str, Any]) -> bytes:
    """
    Tạo file Excel mẫu 2 sheet:
      - Sheet "Nhap_lieu": pre-fill tất cả xã của PGD từ PGD_XA_MAP,
        cột A=STT, B=Xã, C=ĐGD (để trống), D=Ấp/KP (để trống).
        Mỗi xã 1 dòng gợi ý, người dùng thêm dòng nếu cần nhiều ĐGD.
      - Sheet "Danh_muc_thon": danh sách thôn/ấp theo từng xã lấy từ
        HSTD của PGD (dùng pool_thon_cho_xa) và dgd_map hiện có.
        Cột A=Xã, Cột B=Thôn/Ấp. Sắp xếp theo xã.
        Nếu không có dữ liệu HSTD → ghi chú "Chưa có dữ liệu HSTD".
    Trả về bytes để dùng với st.download_button.
    """
    ds_xa = list(PGD_XA_MAP.get(ten_pgd, []) or [])

    rows_nhap: list[dict[str, Any]] = []
    if not ds_xa:
        rows_nhap = [{"STT": 1, "Xã": "", "ĐGD": "", "Ấp/KP": ""}]
    else:
        for i, xa in enumerate(ds_xa, start=1):
            rows_nhap.append({"STT": i, "Xã": xa, "ĐGD": "", "Ấp/KP": ""})
    df_nhap = pd.DataFrame(rows_nhap)

    rows_thon: list[dict[str, Any]] = []
    if not ds_xa:
        rows_thon.append(
            {"Xã": "", "Thôn/Ấp": "(Chưa có cấu hình xã)"}
        )
    else:
        for xa in ds_xa:
            pool = pool_thon_cho_xa(pd.DataFrame(), ten_pgd, xa, dgd_map)
            if pool:
                for thon in pool:
                    rows_thon.append({"Xã": xa, "Thôn/Ấp": thon})
            else:
                rows_thon.append(
                    {"Xã": xa, "Thôn/Ấp": "(Chưa có dữ liệu HSTD)"}
                )
    df_thon = pd.DataFrame(rows_thon)

    buf = BytesIO()
    with pd.ExcelWriter(buf, engine="openpyxl") as w:
        df_nhap.to_excel(w, index=False, sheet_name="Nhap_lieu")
        df_thon.to_excel(w, index=False, sheet_name="Danh_muc_thon")
    return buf.getvalue()
