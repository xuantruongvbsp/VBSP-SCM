...
def phan_loai_gqvl_dp(df_gqvl: pd.DataFrame) -> pd.DataFrame:
    """
    Phân loại dòng GQVL theo nguồn vốn và Mã nhà đầu tư.

    Quy tắc:
      - TW -> TW
      - ĐP + Mã NĐT thuộc danh sách cấp tỉnh -> DP_CAP_TINH
      - ĐP + Mã NĐT không thuộc danh sách hoặc rỗng -> DP_CAP_XA
    Nếu lỗi bất kỳ: trả về bản sao DataFrame với cột Phan_loai_GQVL = UNKNOWN.
    """
    try:
        df_out = df_gqvl.copy()

        # Ưu tiên registry do admin quản lý trong kv_store
        ndt_cap_tinh = db.doc_kv("ndt_dp_list")
        if isinstance(ndt_cap_tinh, list):
            ds_ndt = [str(x).strip().upper() for x in ndt_cap_tinh if str(x).strip()]
        else:
            ds_ndt = []
        if not ds_ndt:
            ds_ndt = [str(x).strip().upper() for x in MA_NDT_CAP_TINH if str(x).strip()]
        set_ndt_cap_tinh = set(ds_ndt)

        nguon = (
            df_out.get("Nguồn vốn", pd.Series(index=df_out.index, dtype="object"))
            .fillna("")
            .astype(str)
            .str.strip()
            .str.upper()
        ...
        ma_ndt = (
            df_out.get("Mã nhà đầu tư", pd.Series(index=df_out.index, dtype="object"))
            .fillna("")
            .astype(str)
            .str.strip()
            .str.upper()
        ...
        mask_tw = nguon.eq("TW")
        mask_dp = nguon.eq("ĐP")
        mask_dp_cap_tinh = mask_dp & ma_ndt.isin(set_ndt_cap_tinh)

        df_out["Phan_loai_GQVL"] = "UNKNOWN"
        df_out.loc[mask_tw, "Phan_loai_GQVL"] = "TW"
        df_out.loc[mask_dp & ~mask_dp_cap_tinh, "Phan_loai_GQVL"] = "DP_CAP_XA"
        df_out.loc[mask_dp_cap_tinh, "Phan_loai_GQVL"] = "DP_CAP_TINH"
        return df_out
    except Exception:
        df_out = df_gqvl.copy()
        df_out["Phan_loai_GQVL"] = "UNKNOWN"
