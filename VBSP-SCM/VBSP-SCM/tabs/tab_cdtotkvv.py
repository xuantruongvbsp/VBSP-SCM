...
def _sub_tong_hop(username: str) -> None:
    ...
    df_th = tong_hop_theo_pgd(df)
    thang = datetime.now().strftime("%m/%Y")

    tong_to   = int(df_th["tong_to"].sum())
    tong_tot  = int(df_th["to_tot"].sum())
    tong_kha  = int(df_th["to_kha"].sum()) if "to_kha" in df_th.columns else 0
    tong_tb   = int(df_th["to_tb"].sum()) if "to_tb" in df_th.columns else 0
    tong_yeu  = int(df_th["to_yeu"].sum())
    ...
    # Bảng hiển thị UI: bỏ cột tình trạng, thêm cột tỷ lệ %
    drop_cols_ui = [
        c for c in ["to_tinh_trang_a", "to_tinh_trang_b", "to_tinh_trang_c"]
        if c in df_th.columns
    ]
    df_th_ui = df_th.drop(columns=drop_cols_ui).copy()
    if "tong_to" in df_th_ui.columns:
        for col_so, col_tl in [
            ("to_tot", "tl_tot"),
            ("to_kha", "tl_kha"),
            ("to_tb", "tl_tb"),
            ("to_yeu", "tl_yeu"),
        ]:
            if col_so in df_th_ui.columns:
                df_th_ui[col_tl] = (
                    df_th_ui[col_so] / df_th_ui["tong_to"].replace(0, pd.NA) * 100
                ).round(1).fillna(0)
    ...
    col_order_ui = [c for c in col_order_ui if c in df_th_ui.columns]
    df_th_ui = df_th_ui[col_order_ui]
    ...
    with col_bang:
        ...
        hien_thi_dataframe_phan_trang(
            ...
            column_config={
                ...
                "tl_tot":         st.column_config.NumberColumn("Tỷ lệ Tốt (%)", format="%.1f%%"),
                "to_kha":         st.column_config.NumberColumn("Khá"),
                "tl_kha":         st.column_config.NumberColumn("Tỷ lệ Khá (%)", format="%.1f%%"),
                "to_tb":          st.column_config.NumberColumn("Trung bình"),
                "tl_tb":          st.column_config.NumberColumn("Tỷ lệ TB (%)", format="%.1f%%"),
                "to_yeu":         st.column_config.NumberColumn("Yếu"),
                "tl_yeu":         st.column_config.NumberColumn("Tỷ lệ Yếu (%)", format="%.1f%%"),
            },
        )

    if df_th is not None and not df_th.empty:
        preview_key = f"preview_cdtotkvv_{thang}"
        pdf_key = f"pdf_cdtotkvv_{thang}"
        show_preview_key = f"show_{preview_key}"
        if show_preview_key not in st.session_state:
            st.session_state[show_preview_key] = False

        col_preview, col_pdf = st.columns(2)
        with col_preview:
            if st.button("🔍 Preview", key=preview_key, use_container_width=True):
                st.session_state[show_preview_key] = not st.session_state[show_preview_key]
        with col_pdf:
            drop_cols = [
                c for c in ["to_tinh_trang_a", "to_tinh_trang_b", "to_tinh_trang_c"]
                if c in df_th.columns
            ]
            df_pdf = df_th.drop(columns=drop_cols).copy()

            if "tong_to" in df_pdf.columns:
                for col_so, col_tl in [
                    ("to_tot", "tl_tot"),
                    ("to_kha", "tl_kha"),
                    ("to_tb", "tl_tb"),
                    ...
                    if col_so in df_pdf.columns:
                        df_pdf[col_tl] = (
                            df_pdf[col_so] / df_pdf["tong_to"].replace(0, pd.NA) * 100
                        ).round(1).fillna(0)
            ...
            col_order = [c for c in col_order if c in df_pdf.columns]
            df_pdf = df_pdf[col_order]

            nut_xuat_pdf(
                df=df_pdf,
                tieu_de=f"Tổng hợp Tổ TK&VV — Tháng {thang}",
                username=st.session_state.get("username", "unknown"),
                prefix_file="CDTOTKVV_TH",
                key=pdf_key,
            )
        ...
        if st.session_state.get(show_preview_key, False):
            ...
            cols_co_san = [c for c in cols_preview if c in df_th.columns]
            if cols_co_san:
                df_preview = df_th[cols_co_san].rename(
                    columns={
                        "ma_dv": "Mã DV",
                        "ten_dv": "Tên đơn vị",
                        "tong_to": "Tổng Tổ",
                        "tong_diem_tb": "Điểm TB",
                        "to_tot": "Tốt",
                        "to_kha": "Khá",
                        "to_tb": "Trung bình",
                        "to_yeu": "Yếu",
                        "to_tinh_trang_a": "TT A",
                        "to_tinh_trang_b": "TT B",
                        "to_tinh_trang_c": "TT C",
                    }
                )
                with st.expander("🔎 Preview tổng hợp", expanded=True):
                    st.dataframe(df_preview, use_container_width=True, hide_index=True)
    
    # Thông tin số đơn vị đã tổng hợp
    so_don_vi_co_data = len(df_th)
    st.caption(f"Đã tổng hợp: {so_don_vi_co_data} / 22 đơn vị")

    xlsx_bytes = xuat_excel({"Tong hop": df_th})
    st.download_button(
        label="⬇️ Xuất Excel",
        data=xlsx_bytes,
