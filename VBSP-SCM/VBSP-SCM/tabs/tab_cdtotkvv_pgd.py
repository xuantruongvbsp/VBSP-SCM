...
def _sub_upload(pgd_user: str, username: str) -> None:
    ...
    if f is not None:
        ...
        if thang_tu_file:
            st.info(f"✅ Nhận diện tháng từ file: **{thang_tu_file}**")
        else:
            st.warning("⚠️ Không nhận diện được tháng từ file. Vui lòng chọn tháng dữ liệu.")

        thang_chon = st.selectbox(
            "Tháng dữ liệu",
            options=ds_thang,
            index=idx_mac_dinh,
            key="cdtotkvv_pgd_thang_du_lieu",
        )

        if st.button("📤 Upload", type="primary", key="cdtotkvv_pgd_btn"):
            path = luu_file_pgd_voi_lich_su(pgd_user, "cdtotkvv", file_bytes_preview, thang_chon)
            ghi_chu = " [tháng nhập tay]" if not thang_tu_file else ""
            db.ghi_audit(
                username,
                "upload_cdtotkvv_pgd",
                f"{pgd_user} · tháng {thang_chon}{ghi_chu}",
            )
            st.success(f"✅ Đã lưu · Tháng {thang_chon} · Đã lưu lịch sử")
