...
def render(tab: DeltaGenerator, **kwargs: dict) -> None:
    ...
    with tab:
        ...
        st.markdown(f"<div class='tq-caption'>Cập nhật: {ngay_cap_nhat} · Hội sở Chi nhánh tỉnh Đồng Nai</div>", unsafe_allow_html=True)
        st.markdown(
            f"""
            <div class="tq-grid">
                <div class="tq-card">
        ...
        if COT_TEN_PGD in df.columns:
            ...
            df_pgd[COT_TEN_PGD] = df_pgd[COT_TEN_PGD].replace("PGD Biên Hòa", "Hội sở Chi nhánh tỉnh")
