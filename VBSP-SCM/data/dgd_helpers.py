...
def _norm_ten_xa(ten: str) -> str:
    """Bỏ prefix Phường/Xã/Thị trấn/TT để so sánh."""
    return re.sub(
...
def _gop_thon_tu_bang(
    ...
    ser_xa = (
        df[col_xa].map(lambda x: _norm_ten_xa(x) if pd.notna(x) else "")
        if col_xa
        else None
    )
    ...
    if not out and ser_xa is not None:
        xa_fallback = df[col_xa].map(lambda x: _norm_ten_xa(x) if pd.notna(x) else "")
        out = _tap_thon(xa_fallback == xa_sel)
