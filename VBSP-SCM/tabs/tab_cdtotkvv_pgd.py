...
def _tinh_no_den_han(_df_bytes: bytes, ngay_tu: str, ngay_den: str) -> bytes:
    """Tính số liệu nợ đến hạn trong khoảng ngày."""
    import pickle, pandas as pd
    df = pickle.loads(_df_bytes)
    from config import COT_NGAY_DH, COT_TEN_TO, COT_DVUT, COT_TEN_KH
    from config import COT_SO_KU, COT_TEN_CT, COT_TONG_DU_NO
    ngay_dh = pd.to_datetime(df[COT_NGAY_DH], errors="coerce")
    t_from  = pd.Timestamp(ngay_tu)
    t_to    = pd.Timestamp(ngay_den)
    mask    = (ngay_dh >= t_from) & (ngay_dh <= t_to)
    cols    = [c for c in [COT_TEN_TO, COT_DVUT, COT_TEN_KH,
                            COT_SO_KU, COT_TEN_CT, COT_NGAY_DH,
                            COT_TONG_DU_NO] if c in df.columns]
...
def render(tab: DeltaGenerator, **kwargs) -> None:
    ...
    with tab:
        ...
        with sub_ndh:
            ...
            st.markdown("#### 📅 Nợ đến hạn")

            # Chọn khoảng thời gian
            khoang_thoi_gian = st.selectbox(
                "Xem nợ đến hạn",
                ["Tháng này", "3 tháng tới", "6 tháng tới", "Trong năm"],
                key="ndh_khoang"
            )

            ngay_tu = date.today()
            nam_hien_tai = ngay_tu.year
            thang_hien_tai = ngay_tu.month

            if khoang_thoi_gian == "Tháng này":
                # Từ hôm nay đến cuối tháng
                ngay_cuoi_thang = monthrange(nam_hien_tai, thang_hien_tai)[1]
                ngay_den = date(nam_hien_tai, thang_hien_tai, ngay_cuoi_thang)
            elif khoang_thoi_gian == "3 tháng tới":
                ngay_den = ngay_tu + timedelta(days=90)
            elif khoang_thoi_gian == "6 tháng tới":
                ngay_den = ngay_tu + timedelta(days=180)
            else:  # Trong năm
                ngay_den = date(nam_hien_tai, 12, 31)

            st.caption(f"Từ {ngay_tu.strftime('%d/%m/%Y')} "
                       f"đến {ngay_den.strftime('%d/%m/%Y')}")

            # Lấy df — đã lọc theo PGD ở cấp ws_operation
            df_ndh_src = kwargs.get("df") if "kwargs" in dir() else None
            if df_ndh_src is None or df_ndh_src.empty:
                st.warning("Chưa có dữ liệu HSTD.")
            elif COT_NGAY_DH not in df_ndh_src.columns:
                st.warning(f"Không tìm thấy cột ngày đến hạn ({COT_NGAY_DH}).")
            else:
                try:
                    raw = _tinh_no_den_han(
                        pickle.dumps(df_ndh_src),
                ...
                if df_ndh.empty:
                    st.success(f"✅ Không có món vay nào đến hạn trong {khoang_thoi_gian.lower()}.")
                else:
                    # KPI
                    ...
                    # Bảng chi tiết
                    hien_thi_dataframe_phan_trang(
                        df_ndh, key="ndh_bang", height=400
                    )
                    ...
                    st.download_button(
                        ...
                        data=buf,
                        file_name=f"NoDenHan_{ngay_tu.strftime('%d%m%Y')}.xlsx",
