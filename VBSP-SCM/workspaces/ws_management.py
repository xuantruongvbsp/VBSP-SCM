...
from tabs import (
    ...
    tab_ban_dai_dien, tab_hoi_doan_the,
)
...
def render(**kwargs):
    ...
    # Lazy render — chỉ render tab đang mở
    if tabs[0].open:
    ...
    if tabs[6].open:
        tab_hoi_doan_the.render(tabs[6], **kwargs)
    if tabs[7].open:
        tab_kehoach.render(tabs[7], **kwargs)
    if tabs[8].open:
        tab_cbtd.render(tabs[8], **kwargs)
    if tabs[9].open:
        tab_quan_ly_dgd.render(tabs[9], **kwargs)
    if tabs[10].open:
        tab_cdtotkvv.render(tabs[10], **dict(kwargs, cdto_mode="cn"))
    if tabs[11].open:
        with tabs[11]:
