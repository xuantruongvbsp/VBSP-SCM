"""Migration 004: Tạo thon_snapshot cho DB từng chạy migration 003 CBTD cũ."""
import sqlite3
...
def upgrade(conn: sqlite3.Connection) -> None:
    # Không xóa cbtd_snapshot legacy: bảng không còn được code mới đọc nhưng vẫn
    # được giữ lại để tránh mất lịch sử nếu một DB đã chạy migration 003 cũ.
    for statement in _SCHEMA_SQL.split(";"):
        sql = statement.strip()
        if not sql:
            continue
        if "idx_thon_snap_pgd_xa" in sql:
            columns = {row[1] for row in conn.execute("PRAGMA table_info(thon_snapshot)")}
            if "ten_pgd" not in columns:
                # DB đã có thon_snapshot schema cũ; migration 005 sẽ nâng cấp.
                continue
        conn.execute(sql)
