    ...
    with ctx:
        ...
        for col in tien_cols:
            if col in df_display.columns:
                df_display[col] = df_display[col].apply(fmt_ty)
        ...
        tom_tat_md = f"""
📌 **{don_vi_ten}** — Tổng dư nợ: **{vn(tong_dn, 0)}** triệu đồng

| Chỉ tiêu | Giá trị |
|---|---|
| Số KH | {fmt_so(so_kh)} |
| Số Tổ | {fmt_so(so_to)} |
| Nợ quá hạn | {vn(nqh, 0)} triệu |
| Nợ khoanh | {vn(nkh, 0)} triệu |
| Cho vay tháng | {vn(ds_cv, 0)} triệu |
| Thu nợ tháng | {vn(ds_thu, 0)} triệu |
    """
