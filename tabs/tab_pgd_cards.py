...
def _render_chart(df_cards: pd.DataFrame) -> None:
    ...
    fig.update_layout(
        ...
        xaxis=dict(
            title=dict(text="Dư nợ (tỷ đồng)", font=dict(size=10)),
            range=[0, max_dn * 1.25],
            gridcolor="#1E3A5F", zeroline=False,
        ),
        xaxis2=dict(
            title=dict(text="NQH%", font=dict(size=10, color="#FFA726")),
            range=[0, max_nqh],
        ...
        xaxis3=dict(
            title=dict(text="BQ/hộ (tr đ)", font=dict(size=10, color="#4DD0E1")),
            range=[0, max_bq],
...
def _render_chart_bq(df_cards: pd.DataFrame) -> None:
    ...
    fig2.update_layout(
        ...
        showlegend=False,
        xaxis=dict(
            title=dict(text="Dư nợ BQ/hộ (triệu đồng)", font=dict(size=10)),
            range=[0, max_bq * 1.25],
            gridcolor="#1E3A5F", zeroline=False,
        ),
        yaxis=dict(gridcolor="#1E3A5F"),
